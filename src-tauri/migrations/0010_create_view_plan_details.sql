...
SELECT
    ...
    COALESCE((p.goal/r.output_amount)*rd.input_amount + 1, 0) required_amount,
	output.id output_item_id,
	output.name output_item_name,
	output.picture output_item_picture,
	own.amount input_amount_owned,
	COALESCE(MAX(((p.goal/r.output_amount)*rd.input_amount + 1) - own.amount, 0), 0) input_need_amount,
